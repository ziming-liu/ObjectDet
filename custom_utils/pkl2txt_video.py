...
def pkl2txt(save_path,pkl_path,json_path):
    ...
    for imgidx in range(num_imgs):
        ...
        frame_id = int(img_id)
        file_name = img_list[imgidx]['filename']
        video_name = file_name.split('/')[1]
        print("dealing with video: {}".format(video_name))
        with open(os.path.join(save_path,video_name+'.txt'),'a') as f:
...
if __name__=="__main__":
    ...
    mode = sys.argv[3]
    if mode =='val':
        json_path = '/home/share2/VisDrone2019/TASK2/VisDrone2018-VID-val/img_size_anno.json'
    elif mode=='test':
        json_path = '/home/share2/VisDrone2019/TASK2/VisDrone2018-VID-test-challenge/img_size_anno.json'
    else:
        raise IOError
    print("Starting")
