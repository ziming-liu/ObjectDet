...
class IPN_sharev2(nn.Module):
    ...
    def __init__(self,
        ...
        super(IPN_sharev2, self).__init__()
    ...
    def forward(self, input):
        ...
        parimad_feats[0] = self.conv_b1(parimad_feats[0])
        #print(parimad_feats[0].shape)
        parimad_feats[1] = self.conv_b2(parimad_feats[1])
        parimad_feats[2] = self.conv_b3(parimad_feats[2])
        outs = []
        for jj in range(self.num_stages): # branch

            for ii in range(self.num_stages-(jj+1),self.num_stages,1): # stage
                layer_name = self.res_layers[ii]
                res_layer = getattr(self, layer_name)
                parimad_feats[jj] = res_layer(parimad_feats[jj])
                if ii == self.num_stages-1 or (ii == self.num_stages-2 and not jj==self.num_stages-1):
    ...
    def train(self, mode=True):
        super(IPN_sharev2, self).train(mode)
        self._freeze_stages()
