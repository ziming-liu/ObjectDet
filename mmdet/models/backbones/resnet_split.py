...
class ResNetS(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            grid_x = x
            main_x = x
            main_x = res_layer(main_x)
            grid_x = self.split(grid_x,num_grid=4**(self.num_stages-i))
            grid_x = res_layer(grid_x)
            grid_x = self.gridcat(grid_x,num_grid=4**(self.num_stages-i))
            x = main_x+grid_x
