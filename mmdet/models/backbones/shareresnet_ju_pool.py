...
class shareResNet_ju_pool(nn.Module):
    """ResNet backbone.
    ...
    def __init__(self,
        ...
        super(shareResNet_ju_pool, self).__init__()
    ...
    def forward(self, input):
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            if i!=0 and i <= len(pyramid) and i<self.num_branch:
                ...
                #else:
                if i==1:
                    x2 = x2+ self.maxpool512_256(x)
                elif i==2:
                    x2 = x2 + self.maxpool1024_256(x)
                elif i==3:
                    x2 = x2 + self.maxpool2048_256(x)
                if i in self.out_indices:
                    outs.append(x2)
        return tuple(outs)

    def train(self, mode=True):
        super(shareResNet_ju_pool, self).train(mode)
        self._freeze_stages()
