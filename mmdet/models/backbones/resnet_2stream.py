    ...
    def forward(self, x):
        ...
        for i in range(self.num_stages-1,-1,-1):
            ...
            if i==self.num_stages-1:
                outs.append(fpn_layer(outsA[i] + outsB[i]))
            else:
                outs.append(fpn_layer(outsA[i] + outsB[i]) + F.interpolate(outs[-1],scale_factor=2))


        return tuple(outs)

    def train(self, mode=True):
        super(ResNet2stream, self).train(mode)
        self._freeze_stages()
