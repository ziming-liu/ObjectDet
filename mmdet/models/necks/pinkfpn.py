    ...
    def forward(self, inputs):
        ...
        for i in range(self.num_stage):
            ...
            for j in range(i, i + num):
                ...
                idx += 1
        for idx_bran in range(self.num_stage-1,0,-1): # 4 3 2 branch
            ...
            for i in range(used_backbone_levels - 1, 0, -1):
                ...
                laterals[i-1] += next
            if idx_bran!=0:
                biger = reshaped_laterals[idx_bran-1][-1]
                smaller = laterals[-1]
                scale_factor = biger.shape[3] / smaller.shape[3]
                next = F.interpolate(
                    smaller, scale_factor=scale_factor, mode='nearest')
                w = min(next.shape[3], biger.shape[3])
                h = min(next.shape[2], biger.shape[2])
                next = next[:, :, :h, :w]
                biger = biger[:, :, :h, :w]
                reshaped_laterals[idx_bran-1][-1] = biger + next
