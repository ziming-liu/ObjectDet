...
class PGCascadeRCNN(BaseDetector, RPNTestMixin,BBoxTestMixin):
    ...
    def __init__(self,
        ...
        super(PGCascadeRCNN, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(PGCascadeRCNN, self).init_weights(pretrained)
    ...
    def extract_feat(self, img):
        ...
        x_large = list(self.neck_shallow(x_large))
        x_mid = list(self.neck(x_mid))
        x_small = list(self.neck_deep(x_small))
        size = len(x_large)
        x = list()
        x.append(x_large[0])
        for i in range(1, size, 1):
            x_mid[i - 1] = (x_mid[i - 1] + x_large[i]) / 2.0
        for i in range(0, size - 1, 1):
            x_mid[i + 1] = (x_mid[i + 1] + x_small[i]) / 2.0
    ...
    def show_result(self, data, result, img_norm_cfg, **kwargs):
        ...
        super(PGCascadeRCNN, self).show_result(data, result, img_norm_cfg,
                                             **kwargs)
